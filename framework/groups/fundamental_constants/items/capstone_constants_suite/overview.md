# Capstone Constants Suite

## One Equation, One Geometry, Zero Free Parameters

Everything in this framework begins with a single equation:
...
| **Characteristic scale a0** | 1.2 x 10^-10 m/s^2 | 1.2 x 10^-10 m/s^2 | < 1% | K^2 x Gm_e/r_e^2 = 16 x g_e |
| **Constitutive law mu(x)** | x/(x+1) | Best fit SPARC data | < 5% residuals | Unique solution: lossless, local, single channel closure |
| **Classical electron radius r_e** | 2.82 fm | 2.818 fm | 0.1% | Three independent routes converge |

## The Fundamental Equations

| Equation | Standard Form | From Topology | Origin |
|---|---|---|---|
| **Inverse square law** | F proportional to 1/r^2 | Field spreading in D = 3: area = 4pi r^2 = Kpi r^2 | Dimensionality |
| **Mass energy equivalence** | E = mc^2 | E = m x c_TetA x c_TetB | Bimetric closure speeds |
| **Force hierarchy** | G_gravity/G_EM approx 10^-43 | alpha^20 approx 10^-43 (gravity traverses 21 levels, EM traverses 1) | Coupling depth difference |
| **Poisson equation** | del^2 Phi = 4pi G rho | del^2 Phi = Kpi x (17/13) x G_bare x rho | Topological integers, scale invariant |
| **3+1 decomposition** | 3 space + 1 time | 6 spatial faces + 2 temporal faces per stellated octahedron | Face structure geometry |
| **Spin 1/2** | S = hbar/2 | Temporal fraction (2/8) x total angular momentum (2hbar) | Vortex aperture at Field Origin |

## The Structural Numbers

Every integer that appears in the framework traces to K = 4 and D = 3:

| Number | What it is | Where it appears |
|---|---|---|
| 1 | Field Origin (existence condition) | In 17 = 16+1, in 13 = 12+1, in f(K) |
| 4 | Faces per tetrahedron (K = D+1) | Simplex theorem, face channels, deflection angle |
| 8 | Binary state space (2^D) | Parity partition vertices, total faces, tetrad legs |
